...
OLD_NLB="E6ENV-WebServersNLB-3dc16ce9b4d9ca48.elb.us-east-1.amazonaws.com"
NEW_NLB="e6lb.dealerfire.com"
OLD_IP1="52.2.222.120"
OLD_IP2="34.206.154.172"
NEW_IP="136.179.129.162"
...
while IFS=, read -r domain; do
  ...
  A_NAME=$(echo "$RECORDS" | jq -r ".ResourceRecordSets[] | select(.Type==\"A\" and (.ResourceRecords | map(.Value) | contains([\"$OLD_IP1\", \"$OLD_IP2\"]))) | .Name")
  if [[ -n "$A_NAME" ]]; then
    echo "🔁 Updating A record $A_NAME to $NEW_IP"
    aws route53 change-resource-record-sets --hosted-zone-id "$ZONE_ID" --change-batch "{
      \"Changes\": [{
        \"Action\": \"UPSERT\",
        \"ResourceRecordSet\": {
